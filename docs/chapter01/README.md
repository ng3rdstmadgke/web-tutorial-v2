...
services:
  backend:
    ...
    ports:
      # コンテナの 8000 番をホストの 8000 番にバインド (ブラウザから http://localhost:8000 で接続できる)
      - "8000:8000"
    volumes:
...
「Dev Container から `localhost:8000` で繋がらない」のはよくあるハマりどころです。**Dev Container は独立したコンテナ**なので、その中の `localhost` は Dev Container 自身を指します。FastAPI コンテナにアクセスするには、サービス名 `backend` を使うか、ホスト OS のブラウザを使ってください。
