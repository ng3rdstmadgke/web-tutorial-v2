...
このチュートリアルは、Chapter14 で GitHub Actions による CI、Chapter15 で EKS 環境へのデプロイを行います。これらのchapterを実施する場合は以下の手順で、リポジトリを自身のGitHubアカウントに複製してください。
...
- Dev Container は `.devcontainer/devcontainer.json` の `runArgs: ["--network=br-web-tutorial-v2-${localEnv:USER}"]` でこのネットワークに参加しています
- FastAPI コンテナは `compose.yaml` の `networks: [devcontainer-nw]` で同じネットワークに参加しています
- 同一ネットワーク内では **compose のサービス名** が DNS で解決されるため、Dev Container から `backend` という名前で FastAPI コンテナに到達できます(`container_name` の `web-tutorial-v2-backend-${HOST_USER}` でも到達可能ですが、短いサービス名を使います)
