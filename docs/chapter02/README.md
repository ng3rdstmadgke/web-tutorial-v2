...
この遠回りをすることで、「なぜ FastAPI のようなフレームワークがあるのか」「なぜ近年は API とフロントエンドを分離するのか」が体感します。

> [!NOTE] すでに Web 開発経験がある方へ
> 「レガシーな Web アプリの実装」のセクションは飛ばしても構いません。ただし「レガシーな Web アプリは何がダメなのか」のセクションは、後続の章にも繋がる考え方なので軽く目を通してください。

### アプリの起動

まずは chapter01 で作成した FastAPIのbackendコンテナを起動しましょう。
...
- **パスパラメータ**: URLのパスに埋め込む値  
例: `https://example.com/users/42` の `42` の部分
- **クエリパラメータ**: URL末尾の `?` 以降に `キー=値&...` の形で付ける値  
例: `https://example.com/users/?name=midori&age=35` の `name=midori&age=35` の部分
...
# 追記 ここから >>>
@app.get("/users/{user_id}")  # `GET /users/{user_id}` というAPIを登録する。 (user_id がパスパラメータ)
def read_user(
    # パスパラメータを受け取る引数。 APIアクセス時に {user_id} に当たる部分の値が渡る。 (/users/42 なら 42 がわたる)
    # 型を int で指定しているので値は int にキャストされる。 (/users/foo は int にキャストできないのでエラーになる)
    user_id: int,  
    # クエリパラメータを受け取る引数。 デフォルト値があるとクエリパラメータ扱いとなる。(?query=midori なら midori がわたる)
    query: str = "",
):
...
```bash
curl "http://backend:8000/users/1?query=hello"
...
> [!NOTE] `tags=["Info"]`
> Swagger UI でエンドポイントをグループ化するための指定です。後で「Legacy」「Modern」のグループも作っていきます。
...
```bash
curl -s "http://backend:8000/info/1?foo=bar" | jq .
# {
#   "url": "http://backend:8000/info/1?foo=bar",
#   "method": "GET",
#   "headers": {
#     "host": "backend:8000",
...
```bash
curl -s -X POST \
    "http://backend:8000/info/100" \
    -H 'Content-Type: application/json' \
    -d '{"foo": "bar", "hoge": "fuga"}' | jq .

# {
#   "url": "http://backend:8000/info/100",
#   "method": "POST",
#   "headers": {
#     "host": "backend:8000",
#     "user-agent": "curl/8.5.0",
...
# Pydantic の BaseModel を継承すると、 フィールドの型を宣言するだけでバリデーションと JSON シリアライズが自動で行われる
...
# response_model: この API が返す型を宣言。 指定された型と異なる値を返すと 500 Internal Server Error になる
@app.get("/api/items/", response_model=ItemsSchema, tags=["Modern"])
async def read_items_api(
    # クエリパラメータを関数の引数として型付きで宣言。
    # Python 3.10 以降は Optional[str] の代わりに str | None と書ける
...
```bash
curl -s "http://backend:8000/api/items/" | jq .
# {
#   "items": [
...
# パスパラメータも型付きで宣言。 /api/items/abc のような不正な値は FastAPI が自動で 422 で弾く
async def read_item_api(item_id: int):
    if item_id not in ITEMS:
        # HTTPException: FastAPI のエラー応答用例外。
        # raise するだけで status_code と detail を含む JSON エラーレスポンスが返る
...
```bash
curl -s "http://backend:8000/api/items/1" | jq .
# {"id": 1, "name": "Apple", "price": 100}

curl -s "http://backend:8000/api/items/100" | jq .
# {"detail": "ID=100 not found"}

curl -s "http://backend:8000/api/items/abc" | jq .
# {
#   "detail": [
#     {
#       "type": "int_parsing",
...
# body: リクエストボディを Pydantic モデルの型で受け取る。 型が違うフィールドや欠損があると 422 で自動的に弾かれる
...
```bash
curl -s -X POST "http://backend:8000/api/items/create/" \
  -H 'Content-Type: application/json' \
...
```bash
curl -s -X DELETE "http://backend:8000/api/items/1/" | jq .
# {"id": 1}
```
