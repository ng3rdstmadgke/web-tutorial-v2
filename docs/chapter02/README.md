...
# @app.get(...): FastAPI インスタンス app に「GET /users/{user_id} を登録」する
@app.get("/users/{user_id}")
def read_user(
    # パスパラメータ (URL の {user_id} にマッチ)。
    # 型を int と書くだけで FastAPI が文字列を整数に自動変換する
    user_id: int,
    # クエリパラメータ (?query=xxx)。 デフォルト値があるとクエリ扱いになる
    query: str = "",
):
