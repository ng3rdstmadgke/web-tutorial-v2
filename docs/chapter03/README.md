...
> [!NOTE] `TIMESTAMPTZ` (timestamp with time zone)
...
> [!NOTE] ポイント解説:
> `OR REPLACE` を付けると、同名のトリガーが既に存在する場合は上書きします(エラーにはなりません)。`CREATE OR REPLACE FUNCTION` と同じ考え方です。PostgreSQL 14 以降でサポートされています。
...
```mermaid
erDiagram
    users ||--o{ items : "1 user が 複数の item を所有"
    users {
        integer id PK
        varchar username UK
    }
...
```mermaid
erDiagram
    users ||--o{ user_roles : "users:user_roles = 1:多"
    roles ||--o{ user_roles : "roles:user_roles = 1:多"
    users {
        integer id PK
        varchar username UK
    }
...
> [!NOTE] DELETE と TRUNCATE の違い  
> どちらも行を削除しますが仕組みが異なります。**全行を消すなら TRUNCATE が高速**、**一部だけ消すなら `WHERE` が使える DELETE** を使う、と覚えておきます。
>
> | 観点 | `DELETE FROM t` | `TRUNCATE t` |
> |---|---|---|
> | 削除範囲 | `WHERE` で**一部だけ**削除できる | **全行**のみ(`WHERE` 不可) |
> | 速度 | 行を1件ずつ走査するため遅い | テーブルを走査せず一気に空にするため速い |
> | ディスク領域 | `VACUUM` するまで解放されない | **即座に**解放される |
> | ロック | 行単位ロックで並行操作と両立しやすい | `ACCESS EXCLUSIVE` で他の操作を全てブロック |
> | トリガー | 行ごとの `ON DELETE` が発火する | `ON DELETE` は発火せず `ON TRUNCATE` のみ |
> | 連番 (IDENTITY) | 採番状態は変わらない | 既定では維持、`RESTART IDENTITY` で先頭に戻る |
> | 外部キー参照 | そのまま実行できる | 参照元も同時に対象にするか `CASCADE` が必要 |
>
> どちらも **トランザクション内なら `ROLLBACK` で取り消せます**(PostgreSQL では TRUNCATE も transaction-safe)。ただし TRUNCATE は MVCC-safe ではないため、TRUNCATE 前のスナップショットを持つ並行トランザクションからは、コミット前でもテーブルが空に見えます。

> [!TIP] 公式ドキュメント:
> - [TRUNCATE | PostgreSQL](https://www.postgresql.org/docs/current/sql-truncate.html)
> - [DELETE | PostgreSQL](https://www.postgresql.org/docs/current/sql-delete.html)
...
> [!NOTE] 用語
> - **主キー** … レコードを一意に識別できる列または列の組み合わせ。値が重複してはなりません。
> - **関数従属性** … 「入力 `X` に対して出力 `Y` が一意に決まる関係」のこと(`Y は X に従属する` と言います)。例えば `社員ID -> 社員名` のように、**社員 ID が決まれば社員名は 1 つに決まる** ような関係を指します。
...
**テーブル内の部分関数従属を解消し、完全関数従属のみのテーブルを作ります。**
...
**非キー列が非キー列に従属しないようにテーブルを分割することで、推移的関数従属を取り除きます。**

> [!NOTE] 用語
> - **推移的関数従属** … テーブル内に存在する段階的な従属関係。**非キー列が別の非キー列に従属している**ことが問題です。
...
> [!NOTE] ORM を使う場合でも裏側で実行される SQL を意識すること
...
> [!NOTE] VS Code の forwardPorts による自動転送
...
> [!NOTE] `updated` カラムの更新はDBトリガーに任せる
...
> [!NOTE] ポイント解説:
> - **`postgresql+psycopg://`** … URL の `+psycopg` は使用するドライバ名です。`psycopg` は v3 を使うときの指定です。古い `psycopg2` を使う場合は `postgresql+psycopg2://` になります。
...
> [!NOTE] ポイント解説:
> `engine.begin()` は「トランザクションを開いて、ブロックを抜けるときに自動 commit する」コンテキストマネージャです。スキーマ変更系の SQL を流すときに便利な書き方です。
>
...
> [!NOTE] `session.refresh()` を呼ばないとどうなる？
