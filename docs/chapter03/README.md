...
- **`image: postgres:18-alpine`** … PostgreSQL 18 の Alpine Linux ベースの公式イメージ。軽量で起動が速い
- **`environment`** … PostgreSQL の **公式イメージは初回起動時** に `POSTGRES_USER` `POSTGRES_PASSWORD` `POSTGRES_DB` を読んで、ユーザー作成・データベース作成を自動で行う
- **`ports: "5432:5432"`** … ホスト OS の 5432 ポートに公開する。ホスト側の GUI ツール（DBeaver、TablePlus など）から接続したいときに便利
- **`networks`** … backend と同じ Docker ネットワークに参加させる。これによりコンテナ間で **コンテナ名** で通信できる
- **永続化ボリュームは設定していない** … この章では学習目的のため、コンテナを停止すると DB の中身は消えます
...
```bash
# DB 接続情報
DB_HOST=web-tutorial-v2-db-${HOST_USER}
DB_PORT=5432
DB_USER=app
...
> `.env.sample` はテンプレートで git にコミットします。学習者は環境ごとに異なる値（`${HOST_USER}` の部分）を埋めた `.env` を生成して使います。`.env` は git に含めません（`.gitignore` で除外）。  
> 一般的にこのパターンは「**接続先や秘密の値はリポジトリに置かない**」というセキュリティ上の慣例として広く使われます。  

### `.env` を生成する

`envsubst` コマンドでテンプレート内の `${HOST_USER}` を実際の値に置換した `.env` を生成します：

```bash
cd $PROJECT_DIR

# .env を生成
envsubst < backend/.env.sample > backend/.env

# 中身を確認
cat backend/.env
# DB_HOST=web-tutorial-v2-db-ktamido
# DB_PORT=5432
# DB_USER=app
# DB_PASSWORD=app_pass
# DB_NAME=app
...
# 確認
echo $DB_HOST
# web-tutorial-v2-db-ktamido
```

> **`.env` は何度でも生成し直せる**  
> 例えば後の章で接続先を変えたいときは、`.env.sample` を書き換えて `envsubst` を実行し直せば最新の `.env` が手に入ります。
...
ホスト OS のブラウザで http://localhost:8888/lab を開いて JupyterLab にアクセスできます。
