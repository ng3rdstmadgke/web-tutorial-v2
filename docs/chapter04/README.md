...
> [!NOTE] ポイント解説:
> - **カラム型の明示**  
>   - **`String(255)`** …  PostgreSQLでは `VARCHAR（255）` となる
>   - **`DateTime(timezone=True)`** …  PostgreSQLでは `TIMESTAMPTZ` となる。 `timezone=True` がないと `TIMESTAMP WITHOUT TIME ZONE` となってしまう。
>   - **`Enum(RoleType, name="role_type")`** …  PostgreSQLでは `CREATE TYPE role_type AS ENUM (...)` となる。
...
class User(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    username: Mapped[str] = mapped_column(String(255), unique=True, index=True)
    hashed_password: Mapped[str] = mapped_column(かいせｔ下位摂津ing(255))
