...
# .env が無ければテンプレートからコピー (Chapter 3 で生成済みならスキップ)
test -f $PROJECT_DIR/backend/.env || cp $PROJECT_DIR/backend/.env.sample $PROJECT_DIR/backend/.env
...
> [!WARNING] シェルを開き直すたびに `export` が必要
...
> [!NOTE] 永続化ボリュームを設定していないので、コンテナを破棄するだけで DB が綺麗になります。
...
# BaseSettings: pydantic-settings の基底クラス。
# インスタンス化時に環境変数を自動で読み込んでフィールドにセットする。
# pydantic-settings は大文字小文字を区別しないので、 db_user は環境変数 DB_USER とマッチする
class Environment(BaseSettings):
    ...
    # @property で SQLAlchemy 用の接続 URL を1箇所にまとめる。
    # Alembic からも FastAPI アプリからも同じロジックが使える
    @property
...
# 引数なしで初期化できるが、mypy/Pylance はそれを認識できないので `type: ignore[call-arg]` で警告を抑制する
...
# postgresql+psycopg://app:app_pass@db:5432/app
```
...
# FastAPI の依存性注入 (Depends) で使うことを想定したジェネレータ関数。
# yield で セッションを渡し、 リクエスト処理が終わったら finally で必ず close する。
# Chapter 5 (CRUD) で def some_route(session: Session = Depends(get_session)) のように使う
...
> [!NOTE] ポイント解説:
> - **カラム型の明示**  
>   - **`String(255)`** …  PostgreSQLでは `VARCHAR(255)` になります
>   - **`DateTime(timezone=True)`** …  PostgreSQLでは `TIMESTAMPTZ` になります。 `timezone=True` がないと `TIMESTAMP WITHOUT TIME ZONE` になってしまいます。
>   - **`Enum(RoleType, name="role_type")`** …  PostgreSQLでは `CREATE TYPE role_type AS ENUM (...)` になります。
...
> [!WARNING] なぜ `alembic.ini` に書かないのか？
...
# Alembic は backend/ ディレクトリ直下で実行されるが、 app.* を import するために
# alembic ディレクトリの 1 階層上 (= backend/) を sys.path に明示的に追加する
...
# Base.metadata = model.py で定義した全モデルのテーブル定義の集合。
# これを Alembic に教えることで autogenerate がモデルと DB の差分を検出できる
...
# alembic.ini の sqlalchemy.url を実行時に環境変数由来の URL で上書きする (公式推奨パターン)
...
> [!NOTE] コラム: Alembic でシードデータを入れる方法もある
...
def seed_roles() -> None:
    ...
    # Chapter 3 で学んだセッションスコープ。 ブロックを抜けるときに close される
...
# このファイルが直接 python -m app.seed で呼ばれたときだけ main() を実行する Python の慣習
...
class User(Base):
    ...
    username: Mapped[str] = mapped_column(String(255), unique=True, index=True)
    hashed_password: Mapped[str] = mapped_column(String(255))
    # Mapped[str | None] で NULL 許容 (プロフィール画像は登録必須にしたくないので Optional)。
    # Text 型: URL は上限を決めかねる長い文字列があり得るため、 長さ制限のない TEXT を利用。
...
```python
# backend/alembic/versions/XXXXXXXXXXXX_add_avatar_url_column_to_users.py
...
> [!WARNING] 教訓:
> `alembic revision --autogenerate` で生成されたファイルは **必ず内容をレビュー** してから適用しましょう。意図しない変更(リネームのつもりが drop + add に化けている、など)が含まれていたり、必要な処理(トリガーの追加・修正など)が抜けていたりすることがあります。
>
