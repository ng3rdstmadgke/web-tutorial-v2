...
| **2xx** | Success | リクエストが**正常に処理**されました |
| **3xx** | Redirection | 別の URL に転送します(Chapter 2 で 302 を使いました) |
...
| **201** | Created | **リソースの新規作成に成功**。POST で使います。レスポンスボディに作成されたリソースを含めるのが一般的です |
| **204** | No Content | 成功したが**返すボディが無い**ケース。DELETE で削除成功時に使います |

#### 3xx: リダイレクト系

| コード | 名前 | 用途 |
|---|---|---|
| **301** | Moved Permanently | リソースが恒久的に別の URL に移動しました |
| **302** | Found | 一時的に別の URL に転送します(Chapter 2 の PRG パターンで使用) |
| **304** | Not Modified | キャッシュが有効。ボディを返さず、クライアント側のキャッシュを使わせます |
...
| **401** | Unauthorized | **認証されていない**(トークンが無い、期限切れ)。Chapter 6 で使います |
| **403** | Forbidden | **認可されていない**(認証済みだが権限が無い)。Chapter 6 で使います |
| **404** | Not Found | リクエストした URL のリソースが存在しない |
| **409** | Conflict | リソースの状態と矛盾する操作(ユーザー名の重複など。本教材では 400 で代用) |
| **422** | Unprocessable Entity | リクエストの**構文は正しいがバリデーションに失敗**しました。FastAPI が Pydantic バリデーションエラー時に自動で返します |
...
| **502** | Bad Gateway | リバースプロキシが背後のサーバーから不正なレスポンスを受けました |
| **503** | Service Unavailable | サーバーが一時的に利用不可(メンテナンス中、過負荷) |
| **504** | Gateway Timeout | リバースプロキシが背後のサーバーからの応答を待ちきれませんでした |
...
- **シグネチャから受け取る型・返す型が明確**
- フィールド欠損や型違反は FastAPI が自動で **`422 Unprocessable Entity`** を返します
- **OpenAPI(Swagger UI)が自動生成** されます

> [!NOTE] Pydantic とは何か(おさらい)
...
この章の後半では、ユーザーを作成・更新する API (`create_user` / `update_user`) を実装します。これらはクライアントから **平文パスワード** を受け取りますが、それを DB に保存する前に必ず変換が必要です。そこでまず、その変換を担うユーティリティをここで用意します。

**パスワードをそのまま DB に保存するのは絶対 NG** です。万が一 DB が漏洩したときに、すべてのユーザーのパスワードがそのまま流出してしまうからです。代わりに **不可逆なハッシュ値** に変換して保存します。ハッシュアルゴリズムには **Argon2** を使います(現在の OWASP 推奨)。
...
> [!NOTE] Argon2 とは
> 2015 年の Password Hashing Competition で優勝したアルゴリズムです。**メモリハードな関数**で GPU/ASIC 攻撃に強く、現代のパスワード保存の業界標準です。長年使われてきた **bcrypt** も依然として安全な選択肢ですが、新規プロジェクトでは Argon2 が推奨されています。
...
# ハッシュを確認
hash
# '$argon2id$v=19$m=65536,t=3,p=4$8MnD9b8Lenlpgc8Ylz0tYw$eaK38WDXVCUpAZsHGseK0MxlQUTK3fZo3CBbuJC0dOA'
...
# ヘルスチェック用にルートだけ残す。実際のリソース操作はすべて router 側に集約する
@app.get("/")
def read_root():
...
- ログイン API(`POST /api/v1/login`)でトークン発行
