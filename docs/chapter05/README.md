...
## 1. backend コンテナで .env を読み込めるようにする

これまで backend コンテナは Hello World を返すだけだったので、DB の接続情報は不要でした。Chapter 5 からはアプリが **DB を叩くようになる** ので、`pydantic-settings` の `Environment()` が起動時に `DB_USER` などの環境変数を要求します。

`compose.yaml` の `backend` サービスに **`env_file`** を追加して、`backend/.env` の中身をコンテナ内の環境変数として渡します。
...
毎回の作業前に、シェル側の環境変数を読み込んでアプリを起動しておきます。

```bash
cd $PROJECT_DIR

# .env をシェルの環境変数として export (シェル毎に必要)
export $(grep -v '^#' $PROJECT_DIR/backend/.env | xargs)

# db / backend を再起動 (compose.yaml の env_file 変更を反映するため)
...
- **`UserBase` を継承** … `username` `avatar_url` は Create / Read で共通なので、ベースクラスにまとめて DRY 原則に従う
- **`password` は `UserCreate` のみ** … 平文パスワードはレスポンスに **絶対に含めてはいけない** ので、`UserRead` には入れない
- **`UserUpdate` の全フィールドが Optional** … 部分更新（クライアントが「password だけ変えたい」など）に対応するため
- **`from_attributes=True`** … `RoleRead.model_validate(role_orm_instance)` のように、**SQLAlchemy のモデルから直接 Pydantic モデルに変換** できるようにするオプション
...
- **`APIRouter`** … FastAPI の「ルーターをグループ化する仕組み」。後の章で `users.router`, `items.router` のようにエンドポイント単位で分割するときの土台になる
- **`prefix="/api/v1"`** … ルーターに登録された全エンドポイントが `/api/v1/...` の URL になる。**API のバージョニングはパスで表現する** のが REST の流儀の 1 つ。将来 v2 を作るときは `/api/v2/...` を別途公開すれば、既存クライアントを壊さずに移行できる
- **Chapter 2 で書いた `/users/{user_id}` などの古いエンドポイント** はもう不要なので、`main.py` の `read_root` を残して他は削除しておきます
...
|---|---|
| `"/users/"` | パス |
| `response_model=UserRead` | レスポンスとして返す型を宣言。SQLAlchemy のモデルを return しても、FastAPI が `UserRead` に変換して返す |
| `status_code=status.HTTP_201_CREATED` | デフォルトは 200 だが、**作成時は 201** が REST の慣習 |

#### `Depends(get_session)`

**FastAPI の依存性注入 (Dependency Injection)**。`session: Session = Depends(get_session)` と書くと、リクエストのたびに `get_session()` が呼ばれてセッションが渡され、レスポンス返却後に自動でクローズされます。

これにより：
- 各エンドポイント内で `SessionLocal()` を直接呼ばなくて良くなる
- テスト時にセッションを差し替えやすい
- リクエスト終了時の `close()` を忘れる事故を防げる

#### `select(...)` と `scalar_one_or_none()`

Chapter 3 で学んだ SQLAlchemy 2.x の SELECT 構文です：

- `select(User).where(User.username == data.username)` … クエリを組み立てる
- `session.execute(...)` … 実行して `Result` を返す
- `.scalar_one_or_none()` … **単一の値を取得（無ければ `None`、2 件以上で例外）**

ユーザー名の重複チェックや、`role_id` から Role を引く部分で活躍します。

#### `session.refresh(user)`

`commit()` 後に `user` インスタンスを **DB から再読み込み** します。これにより `user.id`（自動採番）や `user.created` `user.updated`（DB 側の `CURRENT_TIMESTAMP`）が Python 側に反映されます。
...
```bash
curl -s "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/1" | jq .
...
```bash
curl -s "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/99999" | jq .
...
- **`skip: int = 0`** / **`limit: int = 100`** … クエリパラメータとして受け取り、デフォルト値あり
- **`.scalars().all()`** … モデルのリストを取得（Chapter 3 の SELECT 構文）
- **`.order_by(User.id)`** … 順序を保証するため明示的に id 順で並べる
...
# skip / limit でページング
curl -s "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/?skip=0&limit=10" | jq .
```
...
- **`if data.<フィールド> is not None`** で各フィールドの更新を判定。`UserUpdate` のフィールドはすべて Optional なので、**指定されたフィールドだけを更新** する部分更新になっている
- **`user.roles = roles`** で多対多リレーションを丸ごと差し替え。SQLAlchemy が中間テーブル `user_roles` の差分を計算して自動で INSERT / DELETE してくれる
...
```bash
curl -s -X PATCH "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/1" \
  -H "Content-Type: application/json" \
...
- **`status_code=status.HTTP_204_NO_CONTENT`** … 削除成功時は **204 No Content**（ボディなし）が REST の慣習
- **`-> None`** … レスポンスボディが無いので戻り値も None
- **`session.delete(user)`** で削除。`User` モデルの `cascade="all, delete-orphan"` 設定により、**紐づく Items も自動削除** される（Chapter 3 で設定済み）

### 動作確認

```bash
curl -i -X DELETE "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/1"
