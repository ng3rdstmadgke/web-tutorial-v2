...
```
1. クライアント → POST /api/v1/login (username + password)
2. サーバー: パスワード検証 → JWT を生成
3. サーバー → レスポンス (Set-Cookie: access_token=<JWT>; HttpOnly; SameSite=Lax)
4. 以降のリクエスト: ブラウザが自動で Cookie を付与
5. サーバー: Cookie から JWT を取り出し → 署名検証 → ユーザー特定 → 認可チェック
```
...
- **`create_access_token`**: ペイロードに `sub` (ユーザー名) と `exp` (有効期限) を入れて署名する
- **`decode_access_token`**: 署名検証 + 有効期限チェック。`ExpiredSignatureError` と `InvalidTokenError` を分けて処理
- **`get_current_user`**: Cookie → Authorization ヘッダの fallback でトークンを探す
  - **ブラウザからのリクエスト**: Cookie に入っている（`Set-Cookie` で渡したものをブラウザが自動送付）
  - **curl / API クライアント**: `Authorization: Bearer <token>` ヘッダで送る
...
def login(
    ...
    response.set_cookie(
        ...
        secure=env.cookie_secure,  # 本番は True (HTTPS のみ)
...
- **`response.set_cookie(...)`**: FastAPI の `Response` オブジェクトに対して Cookie をセットする
- **`httponly=True`**: JavaScript の `document.cookie` からアクセスできなくなる → XSS でトークンが盗まれない
- **`samesite="lax"`**: 異なるサイトからの POST リクエストでは Cookie が送られない → CSRF 対策
- **`secure=env.cookie_secure`**: 開発 (HTTP) では False、本番 (HTTPS) では True
- **`max_age`**: Cookie の有効期限（秒）。JWT 自体の `exp` と揃える
- **レスポンスボディにもトークンを含める**: curl や API クライアントは Cookie を使わず `Authorization: Bearer` で送りたい場面がある
- **`GET /me`**: `Depends(auth.get_current_user)` で認証済みユーザーを取得し、 `UserRead` 形式で返す。フロントエンドのヘッダーで「現在ログインしているのは誰か」を表示したいときに使います
...
```bash
curl -i -X POST "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/login" \
...
| 属性 | 値 | 役割 | 防ぐ攻撃 |
|---|---|---|---|
| **`Secure`** | (フラグ) | **HTTPS 通信のときだけ** Cookie を送信する。HTTP (平文) では送らない | **通信経路上の盗聴** (中間者攻撃) |
| **`HttpOnly`** | (フラグ) | **JavaScript (`document.cookie`) から Cookie にアクセスできなくする** | **XSS によるトークン盗難** |
| **`SameSite`** | `Lax` | **別サイトからの POST(フォーム) / fetch / xhr / サブリソース読み込みで Cookie を送らない** | **CSRF (クロスサイトリクエストフォージェリ)** |
...
> **SameSite の他の選択肢**
> - `SameSite=Strict`: 別サイトからの **あらゆるリクエスト** で Cookie を送らない。最も安全だが、外部リンクからサイトに遷移したときにログアウト状態に見えてしまう（UX が悪い）
...
5. サーバーは「ログイン済みユーザーからの正規リクエスト」と認識してしまう
...
本教材では Next.js (`:3000`) と FastAPI (`:8000`) がポート違いなので、**ブラウザは fetch を拒否します**（サーバーに到達はしても、ブラウザがレスポンスを読めない）。
...
A`ccess-Control-Allow-Credentials: true`
...
```bash
# 認証なしでアクセスすると 401
curl -s "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/" | jq .
# {"detail": "Not authenticated"}

# Authorization ヘッダ付きでアクセス
TOKEN=$(curl -s -X POST "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/login" \
  -H 'Content-Type: application/json' \
  -d '{"username": "sys_admin", "password": "admin"}' | jq -r .access_token)
curl -s -H "Authorization: Bearer $TOKEN" "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/" | jq .
...
- **`PermissionType`**: 操作の種類を列挙。1 つのロールが複数の権限を持つ
- **`ROLE_PERMISSIONS`**: 「どのロールがどの権限を持つか」を dict で定義
- **`has_role`**: ユーザーが特定のロールを持つかを判定するシンプルなヘルパー
- **`has_permission`**: ユーザーの全ロールの権限を合算し、要求された権限を**すべて**保有しているかを確認
- **`require_permissions`**: **依存関数を動的に生成する** パターン。`Depends(require_permissions([...]))` のように使う
- **`check_resource_ownership`**: リソースの所有者であることを確認する。**SYSTEM_ADMIN は全アクセス可**（`allow_admin=True` がデフォルト）。所有権チェックが必要な全エンドポイントからこのヘルパーを呼ぶことで、ロジックが 1 箇所に集約される

> **認可の 2 つのレイヤー**  
> 本教材の認可は 2 層に分かれています：
>
> 1. **ロールベースの操作権限** (`require_permissions`): 「この操作タイプ（ITEM_UPDATE など）を実行する権限がロールに含まれているか」。エンドポイントの `Depends` で宣言的にチェック
> 2. **リソースベースの所有権** (`check_resource_ownership`): 「このリソース（item）は自分のものか」。エンドポイント内でリソースを取得した後にチェック
>
> この分離により：
> - 操作権限チェックはデコレータ的に宣言でき、エンドポイントの中身に影響しない
> - 所有権チェックは「リソースを取得してから判定する」必要があるので、エンドポイント内で呼ぶ
> - どちらも 1 箇所に集約されているので、ルール変更時に修正する場所が明確
...
# ユーザー作成を試みると 403
curl -s -X POST "http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1/users/" \
...
- **`check_resource_ownership(owner_id=item.user_id, current_user=current_user)`**: `permissions.py` に定義したヘルパーを呼ぶだけ。所有者チェックのロジックが **1 箇所に集約** されている
- **`has_role(current_user, RoleType.SYSTEM_ADMIN)`**: 一覧 API で「全 Item を返すか自分のだけか」を判定。これも `permissions.py` のヘルパー
- **Item の作成 (`create_item`)**: 所有者チェックは不要（作成するのは必ず自分のアイテム）
- **一覧 API (`read_items`)**: SYSTEM_ADMIN なら全件、一般ユーザーは自分のアイテムだけ WHERE 絞り込み

---

## 12. CORS 設定（フロント連携の準備）

Chapter 12 で Next.js フロントからこの API を叩くことになります。ブラウザの **CORS (Cross-Origin Resource Sharing)** 制約をクリアするために、`main.py` に CORS ミドルウェアを追加しておきます。
...
- **`allow_origins`**: Cookie 付きリクエストを受け付けるオリジン。`*` だと `allow_credentials=True` と併用できないので、明示的に指定する
- **`allow_credentials=True`**: ブラウザが Cookie をリクエストに含めるために必須
- 本番では `allow_origins` にデプロイ先のドメインを設定する
...
```bash
BASE_URL="http://web-tutorial-v2-backend-${HOST_USER}:8000/api/v1"
...
- **CORS 設定**: フロントエンド連携に備えた `allow_credentials=True`
