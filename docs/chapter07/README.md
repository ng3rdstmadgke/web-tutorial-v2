...
def setup_logging() -> None:
    ...
    # イベント生成から JSON 出力までに、 タイムスタンプ追加やレベル追加などの加工を順番に通す
    structlog.configure(
        processors=[
            # Python の contextvars に格納された値 (リクエスト ID など) を自動でログイベントにマージ
            structlog.contextvars.merge_contextvars,
            # ログレベルを付与
            structlog.stdlib.add_log_level,
            # タイムスタンプを ISO 8601 形式 + UTC で付与。
            # タイムゾーン非依存なので異なるリージョンのサーバー間でも時系列が正確に揃う
            structlog.processors.TimeStamper(fmt="iso", utc=True),
            # スタックトレースがあればフォーマット
            structlog.processors.StackInfoRenderer(),
            # 例外情報をフォーマット
            structlog.processors.format_exc_info,
            # 最終的に JSON 文字列として出力 (本番では必須。 開発時は ConsoleRenderer() に切り替えると見やすい)
...
スタックトレースはログに残りますが、ユーザーには**一切漏れません**。運用チームはリクエスト ID でログを引けば詳細を確認できます。
...
`.env.sample` に `LOG_FORMAT` を追加したので、`.env` を再作成してからコンテナを再ビルドします。

```bash
cd $PROJECT_DIR

# .env を再作成(LOG_FORMAT などの追加分を反映。テンプレートをコピー)
cp $PROJECT_DIR/backend/.env.sample $PROJECT_DIR/backend/.env

# コンテナを停止・削除してから再ビルド＆起動
# (db -> migrate でマイグレーション+シードまで自動実行される)
...
curl -s -H "Authorization: Bearer $TOKEN" "http://backend:8000/api/v1/users/" > /dev/null
```
...
```bash
curl -i -H "Authorization: Bearer $TOKEN" "http://backend:8000/api/v1/users/"
# ...
...
```bash
curl -s "http://backend:8000/api/v1/test-error" | jq .
# {"detail": "Internal Server Error"}
```
...
ユーザーにはスタックトレースが**一切漏れていません**が、ログには**完全なトレース**が残っています。これが理想的なエラーハンドリングです。
