...
ナイーブな方法は「テスト関数ごとに DB を DROP → CREATE → マイグレーション → シードデータ投入」ですが、これは非常に遅い。テストが 30 個あれば 30 回 DB を作り直すことになります。
...
def admin_token(client: TestClient) -> str:
    ...
    response = client.post("/api/v1/login", params={"username": "test_admin", "password": "admin_pass"})
...
#### `setup_test_db` (scope="session")

- **pytest セッション全体で 1 回だけ** 実行される
- `app_test` DB を作成 → `Base.metadata.create_all()` でスキーマ作成 → トリガー + シードデータ投入
- **Alembic を使わず `create_all()` を使っている理由**: テスト DB はマイグレーション履歴が不要。モデル定義から直接テーブルを作る方が高速でシンプル
- テスト終了後に `app_test` DB を削除

#### `db_session` (scope="function")

各テスト関数ごとに実行される fixture。処理の流れを DB に発行される SQL と対応付けて見ると仕組みが分かります：
...
SAVEPOINT sa1;                      -- session.commit() の直前に自動生成
INSERT INTO users (...) VALUES (...);
RELEASE SAVEPOINT sa1;              -- session.commit() で SAVEPOINT を確定
SAVEPOINT sa2;                      -- 2 回目の commit があれば同様
UPDATE items SET ... WHERE ...;
RELEASE SAVEPOINT sa2;
ROLLBACK;                           -- fixture: transaction.rollback()
                                    -- → BEGIN 以降の全 SQL (SAVEPOINT 含む) が巻き戻る
```

**ポイント**: `join_transaction_mode="create_savepoint"` により、`session.commit()` が **本来の COMMIT ではなく SAVEPOINT** に差し替えられる。エンドポイントのコードは普通に `commit()` を呼んでいるが、実際にはデータが永続化されない。最後に外側の `ROLLBACK` で全部消える。

テスト対象のコードを **一切変更せずに** 隔離できるのがこのパターンの強みです。

#### `client`

- FastAPI の **依存性オーバーライド** (`app.dependency_overrides`) で `get_session` をテスト用の `db_session` に差し替える
- これにより、エンドポイント内で `Depends(get_session)` で取得されるセッションが「SAVEPOINT 内のセッション」になる

#### `admin_token` / `auth_headers`

- 認証が必要なテストで毎回ログインリクエストを書かなくて済むように、ヘルパー fixture として切り出す
...
class TestLogin:
    ...
    def test_login_wrong_password(self, client: TestClient):
        """パスワードが間違っている場合は 401"""
        response = client.post("/api/v1/login", params={"username": "test_admin", "password": "wrong"})
        assert response.status_code == 401

    def test_login_nonexistent_user(self, client: TestClient):
        """存在しないユーザーでログインすると 401"""
        response = client.post("/api/v1/login", params={"username": "nobody", "password": "pass"})
        assert response.status_code == 401
...
class TestAuthorization:
    ...
    def test_operator_cannot_create_user(self, client: TestClient, auth_headers: dict):
        ...
        # OPERATOR でログイン
        login_res = client.post("/api/v1/login", params={"username": "operator", "password": "pass"})
...
class TestItemCRUD:
    ...
    def test_item_ownership(self, client: TestClient, auth_headers: dict):
        ...
        login_res = client.post("/api/v1/login", params={"username": "other_user", "password": "pass"})
...
全テストが通れば成功です。**2〜3 秒** で 17 個のテストが実行される（DB 作成は 1 回だけ、各テストは ROLLBACK で高速）。
