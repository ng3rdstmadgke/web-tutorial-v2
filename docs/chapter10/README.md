...
services:
  ...
  frontend:
    ...
    ports:
      # コンテナの 3000 番ポートをホスト OS の 3000 番に公開
      - "3000:3000"
    volumes:
...
>     (:3000 / :8000 も公開したままなので、 デバッグ時は各コンテナへ直接アクセスもできる)
>   ```
...
ホスト OS のブラウザで http://localhost:8080 を開きます。create-next-app のデフォルトページ（Next.js のロゴと "Get started" のリンク）が表示されれば成功です。
