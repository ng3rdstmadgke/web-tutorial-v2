...
```bash
# 前章 (Chapter 8) の完成状態から始めます (Chapter 9 は概念を学ぶ読み物章でコード成果物はなし)
...
Chapter 8 まででバックエンドの基礎は一通り揃いました。Chapter 9 で React / Next.js の基礎概念を押さえたので、本格的なフロントエンド実装の最初の一歩はこの Chapter 10 になります。
...
pnpm create next-app@16.2 frontend \
  --ts \
  --tailwind \
  --eslint \
  --app \
...
| `--turbopack` | 開発サーバー・ビルド共に **[Turbopack](https://nextjs.org/docs/app/api-reference/turbopack)** を採用します。Turbopack は Rust 製のバンドラで、従来の Webpack を置き換える Next.js の新しい標準です。差分ビルドが非常に速く、ファイル保存からブラウザ反映までの体感が改善します |
| `--src-dir` | `app/` や `components/` を `src/` ディレクトリの下に配置 |
| `--react-compiler` | **[React Compiler](https://nextjs.org/docs/app/api-reference/config/next-config-js/reactCompiler)** を有効化します。コンポーネントを自動解析して不要な再レンダリングを抑制してくれるコンパイラで、これまで手で書いていた `useMemo` / `useCallback` / `React.memo` を **書かなくてもパフォーマンスが出る** ようになります。Next.js 16 で安定版が利用可能です |
...
> [!TIP] Server Component & Client Component
...
> [!NOTE] Preflight は `@layer base` で読み込まれるので、自分が書いたスタイルが優先される
...
> [!NOTE] shadcn/ui のコンポーネントは「自分のコード」
> `src/components/ui/button.tsx` や `card.tsx` は **コピーされた後は完全に自分のもの** です。「Card の枠線をもっと太くしたい」「Button にスピナーを足したい」のような変更は、このファイルを直接書き換えれば済みます。npm の更新を待つ必要も、ライブラリの API に縛られることもありません。
