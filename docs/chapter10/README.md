...
- `compose.yaml` に `frontend` サービスを追加し、`docker compose up` で **Next.js 開発サーバー** が起動します
- `http://localhost:3000` で Tailwind と shadcn/ui を使ったページが表示されます
...
- **`ENV CI=true`** … pnpm と Next.js に「CI 環境で動作している」と認識させるための環境変数です。これを設定しないと、コンテナ起動時に Next.js 16 が走らせる依存関係チェックの内部で **`pnpm install` が対話プロンプト待ちになり、TTY が無いコンテナでは** ビルドに失敗してしまいます。
- **`corepack enable && corepack prepare pnpm@11.1 --activate`** … pnpm を別途インストールせずに使えるようにする 2 段構成
  - [corepack](https://nodejs.org/api/corepack.html)  
  Node.js v16.9.0 以降に同梱されている **パッケージマネージャ管理ツール**で、追加インストールなしでpnpm や yarn を利用できるようになります。
  - `corepack enable` … pnpm コマンドを利用できる状態にします
  - `corepack prepare pnpm@11.1 --activate` … pnpmを事前ダウンロード＆キャッシュし、デフォルトとして有効化します。
- **`COPY ... pnpm-workspace.yaml ./`** … `pnpm-workspace.yaml` の `allowBuilds` 設定が `pnpm install` 実行時に有効になるよう、依存ファイル群と一緒に先にコピーします
- **`pnpm install --frozen-lockfile`** … `pnpm-lock.yaml` を厳密に守ってインストールします。CI でも同じ挙動になります
- **`--hostname 0.0.0.0`** … Next.js 開発サーバーはデフォルトで `localhost` (127.0.0.1) のみで待ち受けるので、コンテナ外からアクセスできるよう `0.0.0.0` に変更しています

---

## 3. compose.yaml に frontend サービスを追加

ルートの `compose.yaml` に `frontend` サービスを追記します。
...
services:
  ...
  frontend:
    ...
    volumes:
      ...
      # node_modules と .next はコンテナ内のものを優先
...
- **`ports: "3000:3000"`** … コンテナの 3000 番ポートをホスト OS の 3000 番に公開します
- **`volumes: ${HOST_DIR}/frontend:/opt/frontend`** … ソース全体をマウントすることで、ホスト側で編集すると即コンテナ内に反映されます（ホットリロード）
- **`/opt/frontend/node_modules` と `/opt/frontend/.next`** … マウントから **除外** するための anonymous volume 指定です（仕組みは下の補足参照）

#### (補足) なぜ `node_modules` を除外するのか?

ホスト側のディレクトリをそのまま `/opt/frontend` にマウントすると、ホスト側にある `node_modules` でコンテナ内のものが上書きされてしまいます。  
`node_modules` が空だったり、 **OS** や **アーキテクチャ** 依存のバイナリがあったりすると、アプリが動かなくなることがあるため、マウントから除外することでコンテナ内でインストールされたバイナリが使われるようにします。

`.next` は Next.js のビルドキャッシュです。同じく OS / Node のバージョン差で不整合が起きうるので、ホストから切り離します。
...
#### (補足) anonymous volume の初期化動作

`host_path:container_path` の形ではなく **コンテナ側のパスだけを書いた volume** は **anonymous volume（匿名ボリューム）** と呼ばれます。

匿名ボリュームには以下の特徴があります：

- **コンテナ初回起動時**: Docker は **コンテナイメージ内のそのパスの中身を匿名ボリュームにコピーして** 使います。つまり Dockerfile の `RUN pnpm install --frozen-lockfile` で入った `node_modules` がそのままボリュームに入ります。
- **2 回目以降の起動**: Docker が管理する領域（`/var/lib/docker/volumes/...`）に保存され続け、ホスト側の `frontend/node_modules` は参照されません。
- **`docker compose down -v`** を実行すると匿名ボリュームも削除されます。

#### 補足: ネットワーク構成

frontend サービスを追加したことで、コンテナは `Dev Container` を含めて 4 つになりました。Chapter 1 で説明した「Dev Container と FastAPI コンテナが同じ Docker bridge ネットワークに繋がっている」構造に、`db` と `frontend` が加わった形です。

```
                           ホスト OS から見えるポート (localhost:NNNN)
                                :3000        :8000        :5432
                                  ▲            ▲            ▲
                                  │            │            │
┌─ Host OS ───────────────────────┼────────────┼────────────┼─────────────┐
│                                 │            │            │             │
│  ┌─────────────────┐  ┌─────────┴─┐  ┌───────┴──┐  ┌──────┴──────┐      │
│  │ Dev Container   │  │ frontend  │  │ backend  │  │ db          │      │
...
backend と db は Chapter 8 までで既に起動するはずです。frontend を加えて全体を起動します。
...
ホスト OS のブラウザで http://localhost:3000 を開きます。create-next-app のデフォルトページ（Next.js のロゴと "Get started" のリンク）が表示されれば成功です。
...
http://localhost:3000 を再読み込みすると、書き換えた内容が表示されるはずです。

### 解説

- **`export default function Home()`** … React コンポーネントです。ファイル名が `page.tsx` だと、Next.js は **そのコンポーネントを 1 つのページとして扱います**
- **`className`** … React では HTML の `class` 属性ではなく `className` を使います（`class` は JavaScript の予約語のため）
- **`p-8`, `text-3xl`, `font-bold`** … Tailwind CSS のユーティリティクラスです。詳しくは次のセクションで解説します
...
http://localhost:3000/about にアクセスすると About ページが表示されます。
...
- **`<html>` `<body>`** … App Router では `RootLayout` の中に書きます。各ページ側では書きません
...
http://localhost:3000/about を開くと、整ったカード形式で About 情報が表示されます。
...
/* 追加 -->*/
const inter = Inter({  // 英数字フォント
  subsets: ["latin"],
  variable: "--font-inter",
});

const notoSansJP = Noto_Sans_JP({  // 日本語フォント
  subsets: ["latin"],
...
- **`Inter`**（英数字用の sans-serif）… `<body>` 全体や見出しなど、通常テキスト用に使います。`Inter({ subsets: ["latin"], variable: "--font-inter" })` の戻り値は `--font-inter` という CSS 変数を有効化するためのクラス名と、そのフォント設定を一括で扱う Wrapper です
- **`Noto_Sans_JP`**（日本語用の sans-serif）… Inter は日本語グリフを持たないので、日本語表示用に組み合わせます。
- **`Geist_Mono`**（コード表示用の monospace）… `<code>` や `<pre>` で表示されるコードスニペット用の等幅フォントです。
- **`subsets: ["latin"]`** … 取り込むフォントのサブセット。`latin` は基本ラテン文字。Noto Sans JP の日本語グリフはフォントファイル本体に含まれているので、`latin` だけ指定すれば日本語も表示されます
- **`className={\`${inter.variable} ${notoSansJP.variable} ${geistMono.variable}\`}`** … 各フォントは `.variable` プロパティに「CSS 変数を有効化するクラス名」を返します。テンプレートリテラルで連結して `<html>` に渡すことで、3 つの CSS 変数が同時に有効になります


> 別のフォントを使いたいときは、`next/font/google` の import を差し替えるだけで切り替えられます。フォント名は [Google Fonts のサイト](https://fonts.google.com/) で探して、スペースをアンダースコアに置き換えれば API 名になります（例: `Source Sans 3` → `Source_Sans_3`）。
...
- **`--font-sans` を Inter + Noto Sans JP の優先スタックに変更** … 通常テキスト用 (`font-sans` クラスや `<body>` 直下) で使われるフォントです。CSS の `font-family` は **左から順にフォントを試す** 仕組みなので、英数字は Inter、Inter にグリフが無い日本語は Noto Sans JP で表示されます
- **末尾の `sans-serif`** … どちらのフォントも読み込めなかった場合の最終フォールバックです
- **`--font-mono` は Geist Mono のまま** … shadcn init が `var(--font-geist-mono)` を設定済みなので、コード表示用フォントは触りません
