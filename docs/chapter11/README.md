...
│   ├── .env                                # <- .env.sample からコピーして作成 (.gitignore 対象)
...
curl -s http://backend:8000/openapi.json | jq '.info'
# {
#   "title": "FastAPI",
#   "version": "0.1.0"
# }
...
>   **コミットすることをおすすめします**。 生成物ですが、これがあるおかげで `pnpm install` 直後でも型補完が効きます。

---

## 2. Server / Client での API アクセス方針


### 2.1 通信パターンの違い

backend の API への通信は、**どこから呼ぶか**で 2 パターンに分かれます。

```mermaid
sequenceDiagram
    participant B as ブラウザ<br/>(Client Component)
    participant N as proxy (nginx)
    participant F as frontend<br/>(Server Component)
    participant BE as backend (FastAPI)

    Note over B,BE: (a) Server Component から: nginx を介さずコンテナ間で直接
    F->>BE: http://backend:8000/api/v1/...
    BE-->>F: レスポンス

    Note over B,BE: (b) ブラウザから: 同一オリジンの相対パスで叩き、nginx が振り分ける
    B->>N: GET /api/v1/... (相対パス)
    N->>BE: /api/ を backend へ転送
    BE-->>N: レスポンス
    N-->>B: レスポンス

```

- **(a) Server Component から backend へ**  
  **frontend コンテナ -> backend コンテナ** のサーバー間通信なので、**Docker ネットワーク内のサービス名**(`http://backend:8000`)で直接アクセスします。
- **(b) Client Component(ブラウザ) から backend へ**  
  **nginx 経由の同一オリジン**でアクセスします。ブラウザから見ると frontend も backend も同じ `http://localhost:8080` (同一オリジン)なので **Cookie がそのまま送受信され、CORS も不要**です。

つまり、**サーバー側だけ「サービス名のベース URL(環境変数)」が必要**で、クライアント側は相対パスで済みます。


### 2.2 .env.sample を用意する

サーバー側で使う `INTERNAL_API_URL`(コンテナ名)だけを環境変数にします。クライアント側は相対パスなので環境変数は不要です。
...
```bash
# テンプレートをコピーして .env を作る
cp $PROJECT_DIR/frontend/.env.sample $PROJECT_DIR/frontend/.env
...
これで `env.INTERNAL_API_URL` を **型が `string` に絞り込まれた** 値として参照できるようになりました。ここまでで環境変数の設定は完了です。次の 2.6 で frontend コンテナを再起動して反映させます。
 ...
 * - クライアント側 (ブラウザ): nginx リバースプロキシ経由の同一オリジンで `/api/...` を叩くので、ベースURLは不要
 */
...
export function LoginForm() {
  ...
  const onSubmit = async (values: LoginFormValues) => {
    ...
    void data;
    // ログイン成功後はフルナビゲーションでトップへ遷移する。
    // window.location で遷移すると、新しい Cookie 付きのリクエストで認証済みの状態を確実に取得できる
    // (router.push のクライアント遷移はプリフェッチ済みのリダイレクト結果を使い回すことがある)。
...
ログインできても、未ログイン状態でも `/` にアクセスできてしまうと意味がありません。**Next.js の Proxy** を使って、Cookie に `access_token` が無いリクエストは `/login` にリダイレクトするようにします。
...
コンテナをリビルドして再起動します。`migrate` サービスがマイグレーションとシードデータの投入まで自動で行います。
...
# ユーザーが入ったことを確認 (migrate が seed を実行済み)
...
- http://localhost:8080/ にアクセスして、http://localhost:8080/login にリダイレクトされることを確認します
- http://localhost:8080/login で `username: sys_admin` / `password: admin` を入力して送信
- 成功するとサイドバーとヘッダー付きの `/` (Home) に遷移します
- DevTools の **Application** -> **Cookies** -> **http://localhost:8080** で **access_token** Cookie が `HttpOnly` / `SameSite=Lax` で設定されていることを確認
...
1. ログイン後の状態で DevTools -> **Application** -> **Cookies** -> **http://localhost:8080** から `access_token` を手動で削除
2. http://localhost:8080/ にアクセス
3. `proxy.ts` が Cookie 不在を検知し `/login` にリダイレクト
4. 同じく http://localhost:8080/about にアクセスしても `/login` へリダイレクトされる
...
- **Next.js Proxy** (`src/proxy.ts`): Cookie の有無で `/login` にリダイレクト。JWT の中身検証は backend に集約。Node.js ランタイムで動作
