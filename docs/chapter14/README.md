...
[tool.ruff.lint]
# E/F: pycodestyle・Pyflakes（基本）, I: import 整列, B: bugbear（バグになりやすい書き方）, S: flake8-bandit（セキュリティ）
select = ["E", "F", "I", "B", "S"]
# E501（行長）はフォーマッタに任せ、日本語コメントの折返しは強制しない
ignore = ["E501"]

[tool.ruff.lint.flake8-bugbear]
# FastAPI の依存注入は引数デフォルトに関数呼び出しを置くのが正式な書き方なので B008 の対象外にする。
...
> [!NOTE] ポイント解説:
> ruff の `select` に **`"S"`（flake8-bandit）** を入れると、ハードコードされたパスワードや危険な関数呼び出しなどのセキュリティ上の指摘が出ます。  
> 一方で **FastAPI のイディオム**（`Depends(...)` を引数デフォルトに置く）や**テストの `assert`** は誤検知になるため、`extend-immutable-calls` と `per-file-ignores` で対象外にします。  
...
| `actions/setup-node` | Node をセットアップ（依存キャッシュ込み） |
| `pnpm/action-setup` | pnpm を用意 |
| `astral-sh/setup-uv` | uv を用意（キャッシュ込み） |
| `actions/upload-artifact` | 実行結果（Playwright レポート等）を保存 |
| `docker/login-action` / `setup-buildx-action` / `metadata-action` / `build-push-action` | GHCR ログイン / ビルド準備 / タグ生成 / build & push |
| `aquasecurity/trivy-action` | イメージの脆弱性スキャン |
    ...
    steps:
      ...
      - id: meta
        ...
        with:
          ...
          # 2種類のタグを付与
          # type=sha: `sha-1a2b3c4` のようなコミットごとに一意で書き換わらないタグ
          # type=latest: 最新のmainブランチを指す動くタグ （`enable={{is_default_branch}}` でmainブランチの更新でのみ作成）。
          tags: |
            type=sha
      ...
      - name: Scan image (Trivy)
        ...
        with:
          ...
          ignore-unfixed: true   # 修正版が出ている脆弱性のみで失敗させる
          exit-code: "1"
...
> [!NOTE] ポイント解説（Trivy）:
> **[Trivy](https://trivy.dev/)** はビルドしたイメージの CVE（既知脆弱性）をスキャンします。`ignore-unfixed: true` で**修正版が公開されている脆弱性のみ**で失敗させます（ベースイメージ由来で手の打ちようがないものは除外）。`exit-code: "1"` で検出時に CI を止めます。
...
このワークフローは、自分の fork で実際に動かして確認できます。

1. リポジトリを fork する
2. feature ブランチを切って何か変更し、`main` への PR を作る
3. PR ページの **Checks** タブで `backend-quality` / `frontend-quality` / `openapi-drift` / `e2e` / `image` が走り、**グリーン**になることを確認する
4. PR を `main` にマージすると、`image` ジョブが GHCR へ push する
5. リポジトリの **Packages** に `*-backend` / `*-frontend` イメージが現れることを確認する
