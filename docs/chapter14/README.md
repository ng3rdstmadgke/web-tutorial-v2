...
>   `pnpm build` を実行すると `.next/standalone` が生成されます。 `.next/standalone` はそれ単体で実行可能なフォルダとなっており、 `node_modules` を必要としません。  
>   ただし、`public` `.next/static` フォルダは standalone に自動で含まれないため手動でコピーします。 `cp -r public .next/standalone/ && cp -r .next/static .next/standalone/.next/`
...
> [!WARNING] standalone 出力に `.env` を焼き込まない  
> build ステージは `COPY frontend ./` でビルドコンテキストを取り込みます。もしホストの `frontend/.env` が混入すると、standalone 出力がそれを**本番イメージに焼き込んで**しまいます。これは Chapter 1 で作成した `.dockerignore`(`**/.env` を除外)で防いでいます。秘密情報や環境差分は**実行時に環境変数で注入**します。

### 1.3 compose.yaml に `target: dev` を明記
...
uv は依存ツリー全体を解決するため、**直接依存、推移的依存どちらのパッケージも `uv lock --upgrade` `uv lock --upgrade-package <PACKAGE_NAME>` で直せます**。  
...
> - **`tsc --noEmit`**  
>   `tsc --noEmit` は型エラーの有無だけを検査し、JavaScript を出力しません。型チェックを目的に使います。
> - **`next typegen && tsc --noEmit`**
>   Next.js16 は `next dev` や `next build` でルート用の型定義を `.next/types` に生成します。そのため `tsc --noEmit` だけを実行するとルートの型チェックが行われません。  
>   そこで **[`next typegen`](https://nextjs.org/docs/app/api-reference/cli/next#next-typegen-options)** で先にルート型を生成してから `tsc` を走らせます。
> - **`"packageManager": "pnpm@11.5.1`**  
...
> [!TIP] エラーの詳細が気になる方はこちら
> - [Lintエラーの理由と修正方針について | Appendix](../appendix/README.md#Lintエラーの理由と修正方針について)
