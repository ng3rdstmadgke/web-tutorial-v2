...
```bash
git checkout chapter15-start
```

## 完成形

```bash
git checkout chapter16-start
```
---

## はじめに

Chapter14 では、main にマージするたびに本番イメージを **GHCR (GitHub Container Registry)** へ push する CI を作りました。この章はその続きの **CD (Continuous Delivery)** を実装します。
Chapter15ではビルドしたイメージを EKS にデプロイするところまでを実装します。

```mermaid
sequenceDiagram
    participant You as 開発者
    participant GH as GitHub Actions
    participant Git as git: deploy/prodブランチ
    participant Argo as Argo CD
    participant K8s as EKS
    You->>GH: main へ push
    GH->>GH: イメージ build + GHCR push
    GH->>Git: k8s manifestの<br>イメージタグを更新して force push
    Argo->>Git: ポーリングで差分検知
    You->>Argo: ArgoCDの同期を承認
    Argo->>K8s: Sync を wave 順で同期 (db -> migrate -> app)
...
| **Deployment** | アプリの Pod を指定数だけ動かし続ける |
...
flowchart TD
    ...
    JOB["Job: migrate<br>(Sync hook / wave 1)"] -.->|"db 起動後に実行"| SDB
```
...
metadata:
  name: db
  annotations:
    argocd.argoproj.io/sync-wave: '0'   # migrate(wave 1) より先に作る
spec:
...
metadata:
  name: db
  annotations:
    argocd.argoproj.io/sync-wave: '0'   # migrate(wave 1) より先に起動する
spec:
...
### 3.3 マイグレーション (Job / Sync hook)

DB スキーマの適用 (`alembic upgrade head`) と初期データ投入 (`app.seed`) を実行する Job です。

この Job は **db起動後** かつ **アプリ起動** に実行する必要があります。Argo CD の hook と sync-wave でこの順序を実現します。

> [!NOTE] ポイント解説: hook の phase と sync-wave
> **hook**  
> `argocd.argoproj.io/hook` アノテーションを付けたマニフェスト (通常は Job/Pod) を、Argo CD が sync の特定の phase で実行する仕組みです。  
> phase は次の順で実行されます。
>
> | phase | 実行タイミング | 代表的なユースケース |
> | :--- | :--- | :--- |
> | **PreSync** | アプリ本体の同期の**前** | 外部 (既存) DB のマイグレーション |
> | **Sync** | アプリ本体の同期と**同じ phase** | 本体リソースと順序を協調させる処理 |
> | **PostSync** | アプリ本体の同期が**完了した後** | スモークテスト / 結合テスト |
> | **SyncFail** | sync が**失敗**したとき | クリーンアップ処理 |
>
> Argo CD は同一phase内リソースを**sync-wave**の順で適用し、**現在の wave が Healthy になるまで次の wave へ進みません**。

この章では sync-wave を次のように割り当てます。

| wave | リソース | 理由 |
| :--- | :--- | :--- |
| **0** | app-config / db (Service + StatefulSet) | migrate の依存先を先に用意する |
| **1** | migrate Job (Sync hook) | db が Healthy になってから実行する |
| **2** | backend / frontend / Ingress | マイグレーション済みの DB に対して起動する |

> [!WARNING] backend/frontend を migrate より前の wave に置かない
> backend の readiness は `/health` (DB 疎通) です。backend を migrate と同じか前の wave に置くと、「Argo CD は wave 内の全リソースが Healthy になるまで次へ進まない」-> 「backend は未マイグレーションで Healthy になれない」-> 「次の wave の migrate が走らない」というデッドロックに陥ります。backend/frontend は必ず migrate より**後の wave 2** に置きます。
...
metadata:
  ...
  annotations:
    ...
    # 次に migrate を作り直す前に、前回の Job を必ず削除する
...
> [!NOTE] ポイント解説: hook-delete-policy に BeforeHookCreation を使う理由
> Job の `spec.template` は immutable です。`migrate` Job が残ったまま次の sync で別の image に変えようとすると、Argo CD は既存 Job を patch できず `field is immutable` で失敗します。`BeforeHookCreation` は**新しい Job を作る前に前回の Job を必ず削除**するため、前回の sync が成功でも失敗でも、毎回安全に作り直せます。直前の実行 Job は次の sync まで残るので、`kubectl logs` でマイグレーションログを確認できます。
...
metadata:
  name: backend
  annotations:
    argocd.argoproj.io/sync-wave: '2'   # migrate(wave 1) の後に起動する
spec:
  replicas: 1
  selector:
    matchLabels:
...
metadata:
  name: backend
  annotations:
    argocd.argoproj.io/sync-wave: '2'   # backend tier は migrate の後
...
metadata:
  name: frontend
  annotations:
    argocd.argoproj.io/sync-wave: '2'   # frontend tier は migrate の後
...
metadata:
  name: web-tutorial
  annotations:
    argocd.argoproj.io/sync-wave: '2'                       # migrate(wave 1) の後に作る
...
# 必要であれば設定を追加 (/ は JSON Patch のパスで ~1 にエスケープする)
# - op: add
...
# - op: add
#   path: /metadata/annotations/alb.ingress.kubernetes.io~1manage-backend-security-group-rules
#   value: "true"
- op: add
...
images:
  - name: web-tutorial-v2-backend
    newName: ghcr.io/YOUR_GITHUB_USERNAME/web-tutorial-v2-backend  # <- 変更: 自分のGHCRのURL
    newTag: latest
  - name: web-tutorial-v2-frontend
    newName: ghcr.io/YOUR_GITHUB_USERNAME/web-tutorial-v2-frontend  # <- 変更: 自分のGHCRのURL
    newTag: latest
...
# 出力例
  ...
  - host: web-tutorial.example.com
        image: ghcr.io/YOUR_GITHUB_USERNAME/web-tutorial-v2-backend:latest
        image: ghcr.io/YOUR_GITHUB_USERNAME/web-tutorial-v2-frontend:latest
        image: ghcr.io/YOUR_GITHUB_USERNAME/web-tutorial-v2-backend:latest   # migrate Job にも適用される
  replicas: 2
  replicas: 2
```
...
kubectl create secret docker-registry ghcr-secret -n web-tutorial \
  ...
  --docker-password='<GitHub classic PAT>'
```

確認

```bash
kubectl get secret -n web-tutorial app-secret -o json | \
  jq ".data|map_values(@base64d)"
# {
#   "DB_PASSWORD": "xxxxxxxxxxxxxxxxxxxxxxx",
#   "TOKEN_SECRET_KEY": "xxxxxxxxxxxxxxxxxxxxxx"
# }

kubectl get secret -n web-tutorial ghcr-secret -o json | \
  jq -r '.data|map_values(@base64d)|.".dockerconfigjson"' | \
  jq '.'
# {
#   "auths": {
#     "ghcr.io": {
...
```bash
# YOUR_GITHUB_NAME は自分のGitHubユーザー名で置換
argocd repo add https://github.com/YOUR_GITHUB_USERNAME/web-tutorial-v2.git \
...
metadata:
  name: web-tutorial
  namespace: argocd   # ArgoCD の設定は ArgoCDがデプロイされている namespace に作成
spec:
  description: web-tutorial アプリ専用プロジェクト
  sourceRepos:
    # この repo からのデプロイだけを許可する
    - https://github.com/YOUR_GITHUB_USERNAME/web-tutorial-v2.git   # <- 変更: 自分の private リポジトリの URL
  destinations: # デプロイ先のクラスタとnamespace
    - server: https://kubernetes.default.svc
      namespace: web-tutorial
  clusterResourceWhitelist:
    # Clusterスコープの操作は Namespace のみ許可
    - group: ''
      kind: Namespace
...
spec:
  ...
  source:
    repoURL: https://github.com/YOUR_GITHUB_USERNAME/web-tutorial-v2.git  # <- 変更: 自分の private リポジトリの URL
...
sync すると Argo CD は `deploy/prod` を読み、Sync phase を wave 順に同期します (`wave 0: db` -> `wave 1: migrate Job` -> `wave 2: backend / frontend / Ingress`)。各 wave は前の wave が Healthy になってから進みます。
...
これで「main にマージ -> CI が deploy/prod を更新 -> Argo CD を手動 sync で承認・反映」という CD パイプラインが完成しました。全体の流れは次の通りです。
...
main へ push して、CI が `deploy/prod` を更新するところまでを確認します。

```bash
cd $PROJECT_DIR
git checkout main

# CI を発火させる(実変更がなければ空コミットでよい)
git commit --allow-empty -m "ci: trigger deploy"
git push origin main
```

deploy ジョブまで成功するかを確認します ([GitHub CLI](https://cli.github.com/) を使います)。

```bash
# 最新の実行をフォロー(完了まで待つ)。Web UI の Actions タブでも確認できる
gh run watch

# deploy ジョブが deploy/prod を更新したか(newTag が sha-xxxxxxx に変わる)
git fetch origin
git show origin/deploy/prod:k8s/overlays/prod/kustomization.yaml | grep newTag
```

`deploy/prod` が更新されると、Argo CD が差分を検知します。手動 sync で承認・反映します。

```bash
# OutOfSync になっていることを確認
argocd app get web-tutorial

# 手動 sync で承認・反映
argocd app sync web-tutorial

# Synced / Healthy と、反映されたイメージタグを確認
argocd app get web-tutorial
```

`argocd app get` の出力で、backend / frontend の image タグが push した `sha-xxxxxxx` になっていれば、CD パイプラインが期待通り動いています。
...
### sync が Syncing のまま進まないとき

sync 中に **hook (migrate Job) が完了できない**と、sync は `Syncing` のまま止まります。よくある原因は image の pull 失敗 (overlay の image 名/タグが不正) です。まず hook Pod の状態を確認します。

```bash
# migrate(hook) Pod の状態を見る
kubectl get pods -n web-tutorial -l job-name=migrate
# Events で ImagePullBackOff / CreateContainerConfigError などの理由を確認する
kubectl describe pod -n web-tutorial -l job-name=migrate
```

原因 (image 名/タグ・`ghcr-secret` など) を直したうえで、止まっている sync を中断してから再実行します。

```bash
# 進行中の sync を中断する
argocd app terminate-op web-tutorial

# 直した内容で再 sync する。
# hook-delete-policy: BeforeHookCreation により、残った migrate Job は次の作成前に自動削除される。
argocd app sync web-tutorial
```

> [!NOTE] ポイント解説: activeDeadlineSeconds による歯止め
> migrate Job には `activeDeadlineSeconds: 300` を設定しています。image の pull 失敗のように Pod が起動できないケースでも、300 秒で Job が `DeadlineExceeded` で失敗するため、sync が無言でハングし続けるのを防げます。
...
- **マイグレーション**: Argo CD の **Sync hook + sync-wave** (db の後・アプリの前) + `BeforeHookCreation` で、毎回の sync で安全に作り直して実行する。`activeDeadlineSeconds` で無言のハングを防ぐ。
